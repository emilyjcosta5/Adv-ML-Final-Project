...
class Clustering:
    ...
    def _single_point(self, point):
        ...
        classification = bayes_dist['Label'].values[bayes_dist['Dist'].idxmin()]
    ...
    def log_prob(self, theta, K, logD_tc, D_tc, n, t = 1E-1):
        idx = 0
        idx1 = n*(K-1)
    ...
    def closure(self, optimizer, theta, K, logD_tc, D_tc, n): 
        optimizer.zero_grad()
        loss = -(self.log_prob(theta, K, logD_tc, D_tc, n))
        loss.backward() 
        return loss
    ...
    def _2Ddist(self, point0, point1):
        point0 = np.array(point0).reshape(1,2)
        dist =  point0 - point1
        return dist
    ...
    def _bayesian_distance(self, dist_matrix): #point0, point1):
        ...
        # dist_matrix = np.vstack(dist_matrix.values.tolist())
        dist_matrix = np.vstack(dist_matrix.values)
        D = distance.squareform(distance.pdist(dist_matrix)) #returns NxN marix of distances
        logD = np.log(D)
        ...
        scFit = clust.spectral_clustering(logD-D+1E6, n_clusters=K) 

        W_init = torch.tensor(self.one_hot(scFit,3))
        ...
        for t in range(100): 
            optimizer.step(self.closure(optimizer, theta, K, logD_tc, D_tc, n))
        ...
        H = torch.autograd.functional.hessian(self.log_prob(theta, K, logD_tc, D_tc, n), theta).diagonal()
        inv_mass = 1.0/H.abs()
        ...
        params_hmc_nuts = hamiltorch.sample(log_prob_func=(self.log_prob(theta, K, logD_tc, D_tc, n)),
        ...
        acf_mat = np.vstack([self.statsmodel.tsa.stattools.acf(param_trace[:,i], fft=False, nlags=40) for i in range(300)])
