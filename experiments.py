...
def radial_equally_spaced_clusters(n_points=100, proportion=(1,1,1,1), n_clusters=4):
    '''
...
def vertically_elongated_equally_spaced_clusters(n_points=100, proportion=(1,1,1), n_clusters=3):
    '''
    Like linear_equally_spaced_clusters, but the clusters are elongated orthogonal to their separation
    instead of their gap increasing
...
if __name__=="__main__":
    ...
    scenario = linear_equally_spaced_clusters()
    plt.plot(distances,scenario[0],color='r',label='Euclidean Accuracy')
    ...
    plt.xlim([1,20])
    plt.ylim([0,1])
    plt.title('3 Linear Equally Spaced Gaussian clusters')
