...
class Clustering:
    ...
    def _bayesian_distance(self, dist_matrix): #point0, point1):
        ...
        H = torch.autograd.functional.hessian(log_prob, theta).diagonal()
        inv_mass = 1.0/H.abs()
        ...
        params_hmc_nuts, acceptance_rate = hamiltorch.sample(log_prob_func=log_prob,
        ...
        C_trace = torch.stack([extractC(i,n,K) for i in range(2000)]).round()
        ...
        #find min of Dist_matrix and get classification guess from mapC:
        min_dist = np.where(np.vstack([abs(d) for d in dist_matrix]).min(axis=0))
        min_dist_idx = list(min_dist)
        mapC_np = mapC.numpy()

        try: 
            bayes_dist = mapC_np[0, 1]
        except: 
            bayes_dist = None
        return bayes_dist
